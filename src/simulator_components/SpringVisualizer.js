...
export function SpringVisualizer({ active, config, onClick, valueAttributes }) {
  const [{ progress }, set,stop] = useSpring(() => ({ from: { progress: 0 },to:{ progress: 10 }}) )
...
function Spring({ progress, tension, maxTension, springLengthAtRest, layoutClassName }) {
  ...
  const calcScale = React.useCallback(
    x =>{
     ...
     return (springLengthAtRest + tension * (x - 1)) / (springLengthAtRest + maxTension) * (springLengthAtRest + maxTension) / (springHeight)
    },
      ...
      <g transform={`translate(0 ${padding})`}>
        <a.g transform={progress.interpolate(x => `scale(1 ${calcScale(x)})` )}>
          <path
            ...
            stroke="#001858" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
